...
// Plain client — no adapter needed for seeding
...
async function main() {
  ...
  const lecturers = await Promise.all([
    prisma.user.upsert({
      where: { email: 'dr.okafor@mouau.edu.ng' },
      update: {},
      create: {
        email: 'dr.okafor@mouau.edu.ng',
        fullName: 'Dr. Emeka Okafor',
        passwordHash: lecturerPass,
        role: 'lecturer',
        staffId: 'LCT001',
        collegeId: colnas.id,
        departmentId: getDept('CSC').id,
      },
    }),
    prisma.user.upsert({
      where: { email: 'prof.nwosu@mouau.edu.ng' },
      update: {},
      create: {
        email: 'prof.nwosu@mouau.edu.ng',
        fullName: 'Prof. Adaeze Nwosu',
        passwordHash: lecturerPass,
        role: 'lecturer',
        staffId: 'LCT002',
        collegeId: colnas.id,
        departmentId: getDept('MTH').id,
      },
    }),
    prisma.user.upsert({
      where: { email: 'dr.adekunle@mouau.edu.ng' },
      update: {},
      create: {
        email: 'dr.adekunle@mouau.edu.ng',
        fullName: 'Dr. Adekunle Williams',
        passwordHash: lecturerPass,
        role: 'lecturer',
        staffId: 'LCT003',
        collegeId: coleng.id,
        departmentId: getDept('EEE').id,
      },
    }),
    prisma.user.upsert({
      where: { email: 'prof.ekwueme@mouau.edu.ng' },
      update: {},
      create: {
        email: 'prof.ekwueme@mouau.edu.ng',
        fullName: 'Prof. Ngozi Ekwueme',
        passwordHash: lecturerPass,
        role: 'lecturer',
        staffId: 'LCT004',
        collegeId: colmgt.id,
        departmentId: getDept('ACC').id,
      },
    }),
  ]);
  ...
  // Resolve collegeId from department for each student
  ...
  console.log(`  ✓ 1 admin, ${lecturers.length} lecturers, 1 invigilator, ${students.length} students`);
  ...
  for (const deptCode of ['CSC', 'MTH', 'EEE', 'ACC']) {
    ...
    for (const student of deptStudents) {
      for (const course of deptCourses) {
        // Register for own level and one level below (cross-level catch-up)
  ...
  console.log(`  ✓ ${regCount} course registrations`);

  // ── Exam ───────────────────────────────────────────────────────────────────
  console.log('Creating exam...');
  const EXAM_ID  = '00000000-0000-0000-0000-000000000001';
  const lecturer = lecturers[0]; // Dr. Okafor
  const csc301   = getCourse('CSC301');
  ...
  const mcqData = [
    ...
    { body: 'Which sorting algorithm has the best average-case time complexity?',
      options: ['Bubble Sort', 'Insertion Sort', 'Merge Sort', 'Selection Sort'], correct: 2 },
    { body: 'A graph with no cycles is called a ___.',
      options: ['Complete Graph', 'Tree', 'Bipartite Graph', 'Directed Graph'], correct: 1 },
    ...
    { body: 'Which data structure is used for implementing BFS?',
      options: ['Stack', 'Queue', 'Heap', 'Priority Queue'], correct: 1 },
    { body: 'In a max-heap, the root node contains the ___ element.',
      options: ['Smallest', 'Median', 'Largest', 'Random'], correct: 2 },
  ];
    ...
    if (!exists) {
      await prisma.question.create({
        data: {
          ...
          fitbAnswers: {
            create: [{ acceptedAnswer: q.answer, isPrimary: true }],
          },
        },
      });
      qCount++;
  ...
  console.log('\n✅ Seed complete!\n');
  console.log('  ┌──────────────────────────────────────────────────────────────────────┐');
  console.log('  │  ADMIN    admin@mouau.edu.ng               → admin123                 │');
  console.log('  │  LECTURER dr.okafor@mouau.edu.ng           → lecturer123              │');
  console.log('  │  LECTURER prof.nwosu@mouau.edu.ng          → lecturer123              │');
  console.log('  │  LECTURER dr.adekunle@mouau.edu.ng         → lecturer123              │');
  console.log('  │  LECTURER prof.ekwueme@mouau.edu.ng        → lecturer123              │');
  console.log('  │  INVIG    invig1@mouau.edu.ng              → invigilator123           │');
  console.log('  │  STUDENT  alice.obi@student.mouau.edu.ng   → student123               │');
  console.log('  └──────────────────────────────────────────────────────────────────────┘');
  console.log(`\n  📊 ${colleges.length} colleges · ${departments.length} departments · ${courses.length} courses`);
  console.log(`  👥 1 admin · ${lecturers.length} lecturers · 1 invigilator · ${students.length} students`);
  console.log(`  📝 ${regCount} registrations · 1 active exam · ${qCount} questions\n`);
}
