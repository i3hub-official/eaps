<!-- src/lib/components/exam/CreateExamForm.svelte -->
<script lang="ts">
import type { CreateExamPageData, ExamFormResult } from '$lib/types/exam.js';
	import {
		ChevronLeft,
		BookOpen,
		Clock,
	...
	import ExamAuthorityBanner from '$lib/components/ExamAuthorityBanner.svelte';


let {
  data,
  form,
  dashboardHref = '/lecturer'
}: {
  data: CreateExamPageData;
  form: ExamFormResult | null;
  dashboardHref?: string;
	...
	// Lightweight Set wrapper to avoid using the built-in mutable Set directly
	class SvelteSet<T> {
		private s: Set<T>;
		constructor(items?: Iterable<T> | null) {
			this.s = new Set(items as Iterable<T> | undefined);
		}
		has(v: T) {
			return this.s.has(v);
		}
	...
	const LEVELS = $derived((data.levels ?? []).map((l) => l.value).sort((a, b) => a - b));
	let selectedLevels = $state<SvelteSet<number>>(new SvelteSet());
	const allLevels = $derived(selectedLevels.size === LEVELS.length && LEVELS.length > 0);

	function toggleLevel(level: number) {
		const next = new SvelteSet<number>(selectedLevels);
		if (next.has(level)) {
			next.delete(level);
			toast(`${level} Level removed`);
		} else {
			next.add(level);
			toast(`${level} Level added`);
		}
		selectedLevels = next;
	}
	...
	function toggleAll() {
		if (allLevels) {
			selectedLevels = new SvelteSet();
			toast('All levels cleared');
		} else {
			selectedLevels = new SvelteSet(LEVELS);
			toast('All levels selected');
		}
	}

	// ── Departments ───────────────────────────────────────────────────────────
	let selectedDepartments = $state<SvelteSet<string>>(new SvelteSet());
	...
	function toggleDept(id: string) {
		...
		const next = new SvelteSet<string>(selectedDepartments);
		if (next.has(id)) {
			next.delete(id);
			toast(`${dept?.code ?? 'Dept'} removed`);
		} else {
			next.add(id);
			toast(`${dept?.code ?? 'Dept'} added`);
		}
	...
	const selectedCourseObj = $derived(
		(data.courses ?? []).find((c: any) => c.id === selectedCourse)
	);

	const gate = $derived(
  selectedCourse && data.gateByCourseId[selectedCourse]
    ? data.gateByCourseId[selectedCourse]
    : { allowed: true, scope: 'lecturer' as const, activeHolderName: null }
);
	...
	const marksPerQuestion = $derived(questionsToPresent > 0 ? totalMarks / questionsToPresent : 0);
	...
	const durationWarning = $derived(
		durationMinutes > 180
			? 'Very long exam — consider splitting into sections'
			: durationMinutes < 10
				? 'Very short — students may not have enough time'
				: null
	);
	...
	function closeAll() {
		courseOpen = deptOpen = semesterOpen = startOpen = endOpen = sessionOpen = false;
	}
	...
	$effect(() => {
		void selectedCourseObj;
		pulseRow('course');
	});
	$effect(() => {
		void selectedLevels.size;
		pulseRow('levels');
	});
	$effect(() => {
	...
	$effect(() => {
		void allowLateEntry;
		void lateEntryMinutes;
		pulseRow('late');
	});
	$effect(() => {
		void questionsToPresent;
		pulseRow('pool');
	});
</script>
...
<div class="page">
	<div class="page-header">
		<a href={dashboardHref} class="back-link">
		...
		<div class="page-header-main">
			...
			<div class="header-actions">
				<a href={dashboardHref} class="btn ghost">Cancel</a>
				<button type="submit" form="exam-form" class="btn primary" disabled={!gate.allowed}>
					...
					<div class="card-body">
						...
						<div class="field">
							<label
								...
								{:else}
									<span class="field-hint-inline muted"
										>{(data.courses ?? []).length} course{(data.courses ?? []).length !== 1
											? 's'
											: ''} available</span
									>
								{/if}
							...
							{#if form?.errors?.courseId}<span class="field-error">{form.errors.courseId}</span
								>{/if}
						</div>
					...
					<div class="card-body">
						<div class="scope-section">
							<div class="scope-row">
								<div class="scope-label-group">
									...
									{#if selectedLevels.size > 0}
										<span class="count-badge" in:scale={{ duration: 160 }}
											>{selectedLevels.size}</span
										>
									{/if}
								</div>
								...
								{#each LEVELS as level}
									...
									>
										...
										{#if selectedLevels.has(level)}
											<span class="level-check" in:scale={{ duration: 140 }}
							...
							<div class="scope-note" class:active={selectedLevels.size > 0 || allLevels}>
								...
								{:else if selectedLevels.size > 0}✓ {[...selectedLevels]
										.sort((a, b) => a - b)
										.map((l) => `${l}L`)
										.join(', ')} students eligible.
											...
											{:else}
												{#each filteredDepts as d}
													...
													>
														...
														{#if selectedDepartments.has(d.id)}<Check
																size={13}
																class="item-check"
															/>{/if}
													</button>
			...
			<div class="col">
					...
					<div class="card-body">
									...
									<div class="dd-panel dt-panel" transition:fade={{ duration: 100 }}>
										...
										<div class="time-row">
											...
											{#if startDate}<button
													type="button"
													class="dt-done-btn"
													onclick={() => (startOpen = false)}>Done</button
												>{/if}
										</div>
									</div>
								{/if}
									...
									<div class="dd-panel dt-panel" transition:fade={{ duration: 100 }}>
												...
												{:else}
													<button
														...
														class:disabled={startDate
															? new Date(`${fmtDate(endCalYear, endCalMonth, day)}T${endTime}`) <=
										...
										<div class="time-row">
											...
											{#if endDate}<button
													type="button"
													class="dt-done-btn"
													onclick={() => (endOpen = false)}>Done</button
												>{/if}
										</div>
									</div>
								{/if}
						...
						<div class="two-col">
							...
							<div class="field">
												...
												{:else}
													{#each filteredSessions as s}
														<button
															type="button"
															class="dd-item"
															class:active={selectedSession === s.session}
															onclick={() => {
														...
														>
															...
															{#if selectedSession === s.session}<Check
																	size={13}
																	class="item-check"
																/>{/if}
														</button>
								...
								{#if form?.errors?.session}<span class="field-error">{form.errors.session}</span
									>{/if}
							</div>
							...
							<div class="field">
											...
											<div class="dd-list">
												{#each SEMESTERS as s}
													...
													>
														...
														{#if selectedSemester === s.value}<Check
																size={13}
																class="item-check"
															/>{/if}
													</button>
								...
								{#if form?.errors?.semester}<span class="field-error">{form.errors.semester}</span
									>{/if}
							</div>
						</div>
					</div>
				...
				<div class="card">
					<div class="card-header">
						...
						<div>
							<h2>Scoring</h2>
							<p>MOUAU Standard: Exam = 70 marks, CA = 30 marks</p>
						</div>
					</div>
					<div class="card-body">
						<div class="score-grid">
									...
									<div class="pass-bar-wrap">
										<div class="pass-bar-track">
											<div
												...
												style="width:{Math.min(passPercent, 100)}%"
											></div>
										</div>
							...
							<div class="score-field">
								...
								{#if maxViolations <= 2}
									<span class="field-warn" transition:fly={{ y: -4, duration: 140 }}>
										<AlertTriangle size={10} /> Very strict — only {maxViolations} chance{maxViolations >
										1
											? 's'
											: ''}
									</span>
						...
						<div class="qtp-wrap">
							<div class="qtp-header">
								<div class="qtp-label-group">
									<Layers size={14} /><span>Questions Per Student</span>
								</div>
							...
							<div class="qtp-row">
								...
								<div class="qtp-hint-bubble" class:active={questionsToPresent > 0}>
									{#if questionsToPresent > 0}
										Each student gets <strong>{questionsToPresent}</strong> questions &middot;
										<strong>{marksPerQuestion.toFixed(2)}</strong>
										marks each = <strong>{totalMarks}</strong> exam marks total
									{:else}
							...
							<p class="field-hint">
								Total exam marks is always <strong>{totalMarks}</strong>. Marks per question = {totalMarks}
								÷ number of questions.
								{#if questionsToPresent > 0}
			...
			<div class="col">
								...
								<input
									...
									oninput={() =>
										onToggleChange(
											'randomizeOptions',
											randomizeOptions,
											'MCQ choices will be shuffled',
											'MCQ choices in fixed order'
										)}
									class="toggle-cb"
								/>
								<span class="toggle-knob"></span>
								...
								<input
									...
									oninput={() =>
										onToggleChange(
											'showResultAfter',
											showResultAfter,
											'Results hidden until you release them',
											'Students will see their score immediately'
										)}
									class="toggle-cb"
								/>
								<span class="toggle-knob"></span>
				...
				<div class="info-box">
					...
					<div>
						...
						<p>
							After creating the exam you will be redirected to add MCQ and fill-in-the-blank
							questions.
						</p>
					</div>
				</div>
				...
				<div class="summary-card">
					...
					<div class="summary-rows">
						...
						<div class="sum-row" class:sum-row-pulse={changedRow === 'depts'}>
							<span>Departments</span>
							<span class="sum-val" class:sum-val-set={selectedDepartments.size > 0}
								>{deptPreview}</span
							>
						</div>
						<div class="sum-row" class:sum-row-pulse={changedRow === 'session'}>
							<span>Session</span>
							<span class="sum-val" class:sum-val-set={!!selectedSession}
				...
				<div class="form-footer">
					<a href={dashboardHref} class="btn ghost">Cancel</a>
					<button type="submit" class="btn primary" disabled={!gate.allowed}
						>Create Exam &amp; Add Questions &rarr;</button
					>
				</div>
			</div>
		{/if}
...
<style>
	...
	.dd-trigger.open :global(.dd-chevron) {
	...
	.score-input-wrap input.input-warn {
		border-color: #f59e0b;
	}
	.score-input-wrap input.input-error {
		border-color: #ef4444;
	}
	.score-unit {
		position: absolute;
	...
	.toggle-row:hover .toggle-icon-wrap {
		background: var(--lc-soft);
		color: var(--lc-600);
	...
	:global(.dark) .dd-panel {
		box-shadow: 0 8px 24px rgba(0, 0, 0, 0.4);
	}
	:global(.dark) .toggle-knob::after {
		background: #e5e5e5;
	}
</style>
