...
<script lang="ts">
	...
	function compareDescriptors(enrolled: number[], current: number[]): number {
		...
		// If dimensions differ, take the minimum length
		console.warn(
			`Dimension mismatch: enrolled=${enrolled.length}, current=${current.length}. Truncating.`
		);
	...
	let nextCheckIn = $derived(
		lastCheckTime
			? Math.max(0, Math.round((checkInterval - (Date.now() - lastCheckTime.getTime())) / 1000))
			: checkInterval / 1000
	);
</script>
