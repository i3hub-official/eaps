<script lang="ts">
	...
	const NO_FACE_GRACE = 3; // 3 consecutive misses = ~30s before violation
	const WARMUP_MS = 20_000; // no violations for first 20s
	let warmingUp = true;
	let warmupTimer: ReturnType<typeof setTimeout> | null = null;

	const COSINE_SOFT = 0.5;
	const EAR_OPEN = 0.25;
	...
	const statusTitle = $derived(
		status === 'error'
			? 'Camera error — monitoring offline'
			: status === 'warning'
				? 'Face issue detected!'
				: status === 'loading'
					? 'Starting camera…'
	...
	function getModel() {
		...
		modelPromise = (async () => {
			...
			const h = new Human({
				...
				face: {
					enabled: true,
					detector: { maxDetected: 4, minConfidence: 0.5, return: true },
	...
	function meanEAR(mesh: number[][]): number {
		return (ear(mesh, L) + ear(mesh, R)) / 2;
	}
...
<style>
	...
	.video-wrap.border-ok {
		border-color: #22c55e;
		box-shadow: 0 0 0 3px rgba(34, 197, 94, 0.15);
	}
	.video-wrap.border-warn {
		border-color: #f59e0b;
		box-shadow: 0 0 0 3px rgba(245, 158, 11, 0.2);
	}
	.video-wrap.border-err {
		border-color: #ef4444;
	...
	.dot-warn {
		animation: pulse-warn 0.9s ease-in-out infinite;
	}
	.dot-err {
