...
<script lang="ts">
  ...
  const MATCH_THRESHOLD = 0.65; // Adjusted for Human
  const SOFT_THRESHOLD = 0.55; // Adjusted for Human
  ...
  if (browser && !modelLoadingPromise) {
    modelLoadingPromise = (async () => {
      console.log('🚀 Pre-loading Human models for monitoring...');
      ...
      console.log('✅ Models pre-loaded for monitoring');
  ...
  function eyeAspectRatio(mesh: number[][], indices: number[]): number {
    const points = indices.map(i => mesh[i]);
    if (!points[0] || !points[3]) return 1;
    const vertical = dist2d(points[1], points[5]) + dist2d(points[2], points[4]);
    const horizontal = 2 * dist2d(points[0], points[3]);
  ...
  function compareDescriptors(enrolled: number[], current: number[]): number {
    if (enrolled.length === current.length) {
      return cosineSimilarity(enrolled, current);
    }
    // Handle dimension mismatch
    const minLength = Math.min(enrolled.length, current.length);
    const trimmedEnrolled = enrolled.slice(0, minLength);
    const trimmedCurrent = current.slice(0, minLength);
    return cosineSimilarity(trimmedEnrolled, trimmedCurrent);
  }
  ...
  async function runCheck() {
    ...
    try {
      ...
      // ── Continuous Liveness & Antispoof Monitoring ─────────────────────────
      let isLive = true;
      let isReal = true;
      
      if (face.liveness && face.liveness.score !== undefined) {
        isLive = face.liveness.score > 0.55;
      }
      
      if (face.antispoof && face.antispoof.score !== undefined) {
        isReal = face.antispoof.score > 0.55;
      }
      
      // Report spoofing attempts as violations
      if (!isLive || !isReal) {
        monitorStatus = 'warning';
