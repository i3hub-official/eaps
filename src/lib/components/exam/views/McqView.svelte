...
<script lang="ts">
  ...
  function select(optId: string) {
    ...
    onAnswer({ selectedOption: optId, textAnswer: null });
  }

  function getOptionClass(optId: string): string {
    if (!isReview) {
      return selected === optId ? 'selected' : '';
    }
    // In review mode, highlight correct vs chosen
    const isCorrect = question.options.find(o => o.id === optId)?.id === optId &&
      question.options.some(o => o.id === optId); // server would need to mark correct
    if (optId === selected) return 'chosen';
    return '';
  }
</script>
...
<div class="mcq-view">
  ...
  {#if isReview && answer}
    <div class="review-note">
      Your answer: {question.options.find(o => o.id === answer.selectedOption)?.optionText ?? 'None'}
    </div>
  {/if}
</div>
...
<style>
  ...
  .option.selected { border-color: var(--g500, #22c55e); background: var(--g50, #f0fdf4); box-shadow: 0 0 0 3px rgba(34,197,94,0.15); }
  .option.disabled { cursor: default; opacity: 0.8; }
  .option.chosen { border-color: #f59e0b; background: #fffbeb; }
