...
<Sidebar.Root collapsible="icon">
	<Sidebar.Header>
		<div class="flex items-center gap-2.5 px-2 py-1.5 group-data-[collapsible=icon]:justify-center group-data-[collapsible=icon]:px-0">
			...
			<div class="hidden size-8 shrink-0 items-center justify-center rounded-md bg-primary group-data-[collapsible=icon]:flex">
				<ShieldCheck class="size-4.5 text-primary-foreground" />
			</div>
		</div>
	</Sidebar.Header>
							...
							<Sidebar.MenuItem>
								...
								>
									{#snippet child({ props }: { props: Record<string, unknown> })}
										<a href={item.href} {...props} class={menuLinkClass(props.class)}>
