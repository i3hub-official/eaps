<!-- src/routes/lecturer/questions/import/+page.svelte -->
<script lang="ts">
  import type { PageData, ActionData } from './$types';
  import { enhance } from '$app/forms';
  import {
    Upload, FileText, FileSpreadsheet, FileJson, FileCode,
    CheckCircle, AlertCircle, XCircle, Info,
    Download, ChevronLeft, HelpCircle, BookOpen,
    Clock, Loader2, Eye, X
  } from '@lucide/svelte';
  import { fly } from 'svelte/transition';

  let { data, form }: { data: PageData; form: ActionData } = $props();
  const { exams, importHistory } = data;
  ...
  $effect(() => {
    if (form) {
      if (form.success) {
        ...
        showToast(form.message || 'Import successful!', 'success');
  ...
  function downloadTemplate() {
    ...
    const sampleData = [
      ...
      {
        body: 'The square root of 144 is ___',
...
<svelte:head><title>Import Questions — MOUAU eTest</title></svelte:head>
...
<div class="page">
  ...
  <div class="page-header">
    ...
    <div class="header-main">
      <div>
        ...
        <p class="subtitle">Bulk import questions from CSV, Excel, JSON, or TXT files</p>
      </div>
    ...
    <form method="POST" enctype="multipart/form-data" use:enhance={() => {
      ...
      >
        ...
        {:else if uploadStatus === 'success'}
          <div class="success-content">
            ...
            <p>{form?.successCount || 0} questions added successfully</p>
            {#if form?.errors && form.errors.length > 0}
        ...
        {:else if uploadStatus === 'error'}
          <div class="error-content">
            ...
            <h3>Import Failed</h3>
            <p>{form?.error || 'An error occurred during import'}</p>
          </div>
        {:else}
          <div class="file-selected">
            <div class="file-icon" style="color:{getFileTypeColor(selectedFile)};">
              <svelte:component this={getFileIcon(selectedFile)} size={32} />
            </div>
            <div class="file-info">
              <span class="file-name">{selectedFile.name}</span>
              <span class="file-meta">{formatFileSize(selectedFile.size)}</span>
            </div>
      ...
      <div class="options-grid">
        <div class="option-group">
          <label for="examId">Target Exam <span class="optional">(optional)</span></label>
          <select id="examId" bind:value={selectedExam}>
          ...
          <span class="hint">If not specified, will use the exam code from each row</span>
        </div>
      </div>
      ...
      >
        {#if uploadStatus === 'uploading'}
          <Loader2 size={18} class="spin" /> Importing...
        {:else}
  ...
  {#if importHistory && importHistory.length > 0}
    <div class="history-section">
      <div class="history-header">
        ...
        <span class="history-count">{importHistory.length} imports</span>
      </div>
  ...
  <div class="info-box">
    ...
    <div class="info-content">
      ...
      <ul>
        <li><strong>Required columns:</strong> <code>body</code> (question text), <code>type</code> (mcq, true_false, fill_in_the_blank), <code>examCode</code></li>
        <li><strong>Optional columns:</strong> <code>marks</code>, <code>topic</code></li>
        <li><strong>For MCQ:</strong> Provide <code>option1</code>, <code>option2</code>, <code>option3</code>, <code>option4</code> and <code>correctIndex</code> (0-based)</li>
        <li><strong>For True/False:</strong> Provide <code>correctAnswer</code> (true or false)</li>
        <li><strong>For Fill in the Blank:</strong> Provide <code>acceptedAnswers</code> (comma separated)</li>
        <li><strong>Exam code:</strong> Use the course code (e.g., <code>CSC101</code>, <code>PHY101</code>)</li>
      </ul>
    </div>
  </div>
</div>
...
<style>
  /* Same styles as student upload page */
  /* Reuse the styles from the student upload page with minor adjustments */
