...
<script lang="ts">
  ...
  let editingQuestion = $state<any | null>(null);
  ...
  $effect(() => {
    if (form?.success) {
      const msgs: Record<string, string> = {
        create:        'Question added',
        edit:          'Question updated',
        delete:        'Question deleted',
        reorder:       'Order saved',
        import:        `${form?.count ?? 'N'} questions imported`,
      };
  ...
  function openEdit(q: any) {
    ...
    if (editIsTF) {
      ...
      editOptions = q.options.map((o: any) => o.optionText);
      editCorrect = q.options.findIndex((o: any) => o.isCorrect);
  ...
  // ── Helpers ────────────────────────────────────────────────────────────────
  function qLabel(q: any) {
    const isTF = q.options.length === 2 && q.options.some((o: any) => o.optionText === 'True') && q.options.some((o: any) => o.optionText === 'False');
    return isTF ? 'T/F' : 'MCQ';
  }
...
<svelte:head><title>Questions — {exam.course.code} | Lecturer</title></svelte:head>
...
<div class="qb-page">
  ...
  <header class="qb-header">
    <div class="header-top">
      <a href="/lecturer/exams/{exam.id}" class="back-link"><ChevronLeft size={14} strokeWidth={2.5} /> Back to Exam</a>
    </div>
  ...
  {#if openModal === 'add_mcq'}
    <div class="modal" transition:fly={{ y: 24, duration: 260, easing: cubicOut }} role="dialog" aria-modal="true" aria-label="Add MCQ">
      ...
      >
        ...
        <input type="hidden" name="correctIndex" value={mcqCorrect} />
  ...
  {:else if openModal === 'edit' && editingQuestion}
    <div class="modal" transition:fly={{ y: 24, duration: 260, easing: cubicOut }} role="dialog" aria-modal="true" aria-label="Edit question">
      ...
      >
        ...
        <input type="hidden" name="type" value={editIsTF ? 'true_false' : 'mcq'} />
        <div class="field">
        ...
        {:else}
          <div class="options-section">
            ...
            {/if}
            <input type="hidden" name="correctIndex" value={editCorrect} />
          </div>
        {/if}
