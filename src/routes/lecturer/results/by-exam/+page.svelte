...
<script lang="ts">
  ...
  import { 
    ChevronDown, ChevronRight, Search, FileSpreadsheet, FileText, Eye,
    FileSearch, Users, CheckCircle2, XCircle, Clock, Award
  } from '@lucide/svelte';
  ...
  function exportExam(examId: string, format: 'excel' | 'pdf') {
    goto(`/lecturer/results/export?examId=${examId}&format=${format}`);
  }
</script>
        ...
        <div class="exam-group">
          ...
          >
            ...
            <div class="exam-actions" onclick={(e) => e.stopPropagation()}>
              ...
              <button
                class="btn-icon"
                onclick={() => exportExam(group.examId, 'excel')}
              ...
              <button
                class="btn-icon"
                onclick={() => exportExam(group.examId, 'pdf')}
                title="Export to PDF"
                type="button"
              >
                <FileText size={14} />
