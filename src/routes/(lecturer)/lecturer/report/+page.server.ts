...
export const load: PageServerLoad = async ({ locals, url }) => {
    ...
    type CellData = {
        ...
        grade: string
        pass: boolean
        submittedAt: string | null // Changed from Date to string for serialization
    }
    ...
    for (const session of sessions) {
        ...
        scoreMap.get(sid)!.set(session.assessmentId, {
            ...
            pass: isPassing(pct),
            submittedAt: session.submittedAt ? session.submittedAt.toISOString() : null, // Fixed Date serialization here
        })
    }
