...
<script lang="ts">
  import {
    ...
    Plus, ShieldAlert, CreditCard, CheckCircle2,
  ...
  const grouped = $derived(() => {
    ...
    for (const r of registrations) {
      if (q && !r.courseCode.toLowerCase().includes(q) &&
              !r.courseTitle.toLowerCase().includes(q) &&
              !r.department.toLowerCase().includes(q)) continue;
  ...
  const TYPE_META: Record<string, { label: string; color: string; bg: string; headBg: string }> = {
    normal:     { label: 'Normal',     color: 'var(--green-700)',  bg: 'var(--green-soft)',          headBg: 'var(--green-soft)' },
    carry_over: { label: 'Carry-Over', color: '#b45309',           bg: 'rgba(245,158,11,.10)',        headBg: 'rgba(245,158,11,.13)' },
    borrowed:   { label: 'Borrowed',   color: '#4338ca',           bg: 'rgba(99,102,241,.08)',        headBg: 'rgba(99,102,241,.12)' },
  };

  function examStatusStyle(s: string) {
    switch (s) {
      case 'active':    return { color: 'var(--green-700)', bg: 'var(--green-soft)' };
      case 'scheduled': return { color: '#1d4ed8',          bg: 'rgba(37,99,235,.1)' };
  ...
  function regStatusStyle(status: string) {
    switch (status) {
      ...
      case 'pending':  return { text: 'Pending',  color: '#b45309',          bg: 'rgba(245,158,11,.12)' };
      case 'rejected': return { text: 'Rejected', color: '#b91c1c',          bg: 'rgba(220,38,38,.1)' };
  ...
  const creditColor = creditPercentage > 90 ? '#dc2626' : creditPercentage > 75 ? '#f59e0b' : 'var(--green-600)';
</script>
  ...
  {#if hasAny}
    <div class="summary-strip">
      ...
      <div class="credit-progress">
        <div class="progress-bar">
          <div class="progress-fill" style="width:{Math.min(creditPercentage, 100)}%; background:{creditColor}"></div>
        </div>
      ...
      {#if carryOverCount > 0}
        <div class="type-stat" style="color:#b45309; background:rgba(245,158,11,.12)"
      ...
      {#if borrowedCount > 0}
        <div class="type-stat" style="color:#4338ca; background:rgba(99,102,241,.1)"
    ...
    {#if meta.limits.hasReachedCarryOverLimit}
      <div class="notice notice-warning">
        ...
        <div>You have reached the carry-over course limit ({meta.limits.maxCarryOver}). No more carry-over courses can be added.</div>
      </div>
    {/if}
    ...
    {#if meta.limits.hasReachedBorrowedLimit}
      <div class="notice notice-warning">
        <AlertTriangle size={14}/>
        <div>You have reached the borrowed course limit ({meta.limits.maxBorrowed}). No more borrowed courses can be added.</div>
      </div>
    {/if}
    ...
    {#if !meta.canAddMore}
      <div class="notice notice-warning">
        <XCircle size={14}/>
        <div>You have reached the maximum credit limit ({meta.maxCredits} CU). No more courses can be added.</div>
      </div>
    {/if}
    ...
    {:else if hasPending}
      <div class="notice notice-amber">
        <ShieldAlert size={14}/>
        <div>Some carry-over courses are <strong>pending approval</strong> from your academic office.</div>
      </div>
    {/if}
    ...
    {#if academicSemester && !academicSemester.regOpen}
      <div class="notice notice-info">
        <Info size={14}/>
        <div>Registration is currently <strong>closed</strong> for {academicSemester.label}. Please contact your academic office.</div>
      </div>
    {/if}
  {/if}
...
<div class="group-header" style="background:{tm.headBg}">
  ...
  <span class="group-count" style="color:{tm.color}">{list.length} course{list.length !== 1 ? 's' : ''}</span>
                ...
                {#each list as reg (reg.id)}
                  ...
                  <div class="course-card" style="--accent:{tm.color}; --accent-bg:{tm.bg}">
                    ...
                    <div class="card-head">
                      ...
                      <span class="status-chip" style="color:{rs.color}; background:{rs.bg}">{rs.text}</span>
                    </div>
                      ...
                      <div class="exams-block">
                        {#each reg.exams as exam}
                          ...
                          <div class="exam-row">
                            ...
                            {/if}
                            <span class="exam-status-badge" style="color:{es.color}; background:{es.bg}">{exam.status}</span>
...
<style>
  ...
  .stat-card { display: flex; align-items: center; gap: 0.4rem; }
  ...
  .stat-divider { width: 1px; height: 24px; background: var(--color-border); flex-shrink: 0; }
  
  .type-stat {
    display: inline-flex; align-items: center; gap: 0.3rem;
    font-size: 0.67rem; font-weight: 700; padding: 0.2rem 0.55rem; border-radius: 999px;
  }
  ...
  /* Credit progress */
  .credit-progress { display: flex; flex-direction: column; gap: 0.2rem; min-width: 120px; }
  ...
  .remaining-text { font-size: 0.6rem; color: var(--color-muted); }

  /* Notices */
  .notice {
    display: flex; align-items: flex-start; gap: 0.6rem;
    padding: 0.7rem 0.875rem; border-radius: 0.6rem; border: 1px solid;
  ...
  .notice-warning { background: rgba(245,158,11,.07); border-color: rgba(245,158,11,.3); color: #92400e; }
  .notice-info    { background: rgba(59,130,246,.07); border-color: rgba(59,130,246,.25); color: #1e40af; }
  ...
  .search-wrap {
    ...
    max-width: 360px; color: var(--color-muted);
  }
  .search-wrap input {
  ...
  .empty-cta {
    ...
    padding: 0.55rem 1rem; border-radius: 0.5rem;
    background: var(--green-600); color: #fff;
    font-size: 0.8rem; font-weight: 700; text-decoration: none; transition: background 0.15s;
  }
  ...
  .group-label { font-size: 0.75rem; font-weight: 800; }
  .group-count { font-size: 0.67rem; font-weight: 600; opacity: 0.8; }
  .group-cu    { font-size: 0.67rem; font-weight: 700; margin-left: auto; }
  ...
  .card-code-row { display: flex; align-items: baseline; gap: 0.4rem; }
  .course-code { font-size: 0.75rem; font-weight: 800; font-family: monospace; letter-spacing: 0.02em; }
  .course-cu-badge { font-size: 0.6rem; font-weight: 700; color: var(--color-muted); }

  .status-chip {
    font-size: 0.58rem; font-weight: 800;
    text-transform: uppercase; letter-spacing: 0.04em;
    padding: 0.1rem 0.38rem; border-radius: 0.25rem;
  ...
  .exam-row {
    ...
    font-size: 0.71rem; padding: 0.3rem 0.5rem;
    border-radius: 0.35rem; background: var(--color-bg);
  }
  .exam-dot { width: 5px; height: 5px; border-radius: 50%; flex-shrink: 0; }
  .exam-title-text {
  ...
  .exam-date {
    display: inline-flex; align-items: center; gap: 0.2rem;
    color: var(--color-muted); white-space: nowrap; font-size: 0.65rem;
  }
  .exam-status-badge {
    font-size: 0.58rem; font-weight: 800;
    text-transform: uppercase; letter-spacing: 0.03em;
    padding: 0.08rem 0.3rem; border-radius: 0.2rem; white-space: nowrap;
  }
  .exam-go {
    display: inline-flex; align-items: center; gap: 0.2rem;
  ...
  .no-exam {
    ...
    font-size: 0.67rem; color: var(--color-muted);
    padding-top: 0.125rem; margin-top: auto;
  }
  ...
  @media (max-width: 600px) {
    ...
    .credit-progress { min-width: 100px; }
