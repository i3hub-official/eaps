...
<script lang="ts">
	...
	function onFullscreenChange() {
		if (step !== 'exam' || !fullscreenReady) return;
	...
	// ─── Enter/Resume (user gesture handlers) ─────────────────────────────────
		...
		<div class="lobby-screen">
			<div class="lobby-card">
				<div class="lobby-header">
					...
					<h1 class="lobby-title">All checks passed</h1>
				...
				<div class="rules-footer">
					<div class="lobby-actions">
						...
						{#if isResume}
							<button class="btn-enter-exam" onclick={handleResumeExam}>
								<Shield size={16} /> Resume Exam &amp; Go Fullscreen
							</button>
						{:else}
							<button class="btn-enter-exam" onclick={handleEnterExam}>
								<Shield size={16} /> Enter Exam &amp; Go Fullscreen
							</button>
						{/if}
					</div>
				</div>
							...
							{#if question.type === 'mcq'}
								<div class="options-list">
									{#each question.options as opt, i}
										...
										>
											<span class="option-letter" class:selected>{LETTERS[i] ?? i + 1}</span>
							...
							{:else}
								<div class="fitb-wrap">
									<label class="fitb-label" for="answer-input">
										{question.type === 'essay' ? 'Your answer:' : 'Fill in the blank:'}
									</label>
				...
				{#if !questionLoading || question}
					<div class="question-footer">
						<div class="nav-buttons-wrapper">
							...
							>
								<ChevronLeft size={22} /> Previous <kbd>P</kbd>
							</button>

							<div class="footer-center">
								<span class="question-counter-lg">{currentIndex + 1} / {totalQuestions}</span>
								{#if !isOnline}<span class="offline-text">Offline</span>{/if}
							</div>
			...
			{:else if result}
				<div class="score-hero" style="--grade-color:{gradeColour(result.passed)}">
					...
					<div class="score-verdict" style="color:var(--grade-color)">
						{#if result.passed === true}<Trophy size={18} /> Passed
						{:else if result.passed === false}<XCircle size={18} /> Not passed
						{:else}Awaiting grading{/if}
					</div>
				</div>
