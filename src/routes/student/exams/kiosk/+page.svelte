...
<script lang="ts">
  ...
  import {
    ...
    mergeServerAnswers, pendingAnswers,
...
// ─── Lobby state ──────────────────────────────────────────────────────────
  ...
  type ClientQuestion = {
    ...
    body: string; imageUrl: string | null; marks: number;
  ...
  async function handleViolation(type: string) {
    ...
    try {
      const res = await fetch(`/api/exam/${data.examId}/violation`, {
        ...
        body: JSON.stringify({ sessionId, flagType: type }),
      });
      if (res.ok) {
        const json = await res.json();
        if (typeof json.violationCount === 'number') violationCount = json.violationCount;
        violationAction = json.action ?? 'warning';
        if (json.action === 'auto_submitted') { violationVisible = true; await submitExam(); return; }
        if (json.action === 'exam_paused') { notifyOpener('flagged'); stopAllTimers(); step = 'flagged'; return; }
      } else { violationAction = 'warning'; }
    } catch { violationAction = 'warning'; }
    finally { violationInFlight = false; }
  ...
  async function syncOfflineQueue() {
    ...
    isSyncing = true;
    const batch = [...offlineQueue]; const failed: typeof offlineQueue = [];
    ...
    offlineQueue = failed; isSyncing = false;
  }
  ...
  async function fetchResult() {
    if (!sessionId) return;
    resultLoading = true; resultError = '';
    try {
    ...
    } catch { resultError = 'Network error loading result.'; }
  ...
  function startRedirectCountdown() {
    ...
    redirectInterval = setInterval(() => {
      ...
      if (redirectSecsLeft <= 0) {
        clearInterval(redirectInterval!); redirectInterval = null;
  ...
  function buildPersistedState(): PersistedSession {
    return {
      sessionId: sessionId!, examId: data.examId,
  ...
  async function fetchQuestion(idx: number) {
    if (!sessionId) return;
    questionLoading = true; questionError = ''; question = null;
    try {
      const res = await fetch(`/api/exam/${data.examId}/question?index=${idx}&sessionId=${sessionId}`);
      if (res.status === 409) { void handleServerSubmitted(); return; }
      if (res.status === 423) { notifyOpener('flagged'); stopAllTimers(); step = 'flagged'; return; }
      if (!res.ok) {
        const j = await res.json().catch(() => ({}));
        questionError = j.message ?? `Failed to load question ${idx + 1}`; return;
      }
      const payload = await res.json();
      question = payload.question; totalQuestions = payload.total; currentIndex = idx;
      persistSession(buildPersistedState());
    } catch { questionError = 'Network error — press R to retry'; }
  ...
  async function saveAnswer(idx: number, value: string) {
    ...
    } catch { offlineQueue = [...offlineQueue, { index: idx, questionId, value }]; }
  }
  ...
  async function startExam() {
    ...
    try {
      ...
      sessionId = json.sessionId; totalQuestions = json.totalQuestions; timeRemaining = json.timeRemaining;
      ...
      if (stored) {
        ...
        answers = json.serverAnswers ?? {}; currentIndex = json.currentIndex ?? 0;
      }
      if (stored) {
        const serverKnown = new Set(Object.keys(json.serverAnswers ?? {}).map(Number));
        const pending = pendingAnswers(stored, serverKnown);
        for (const item of pending) offlineQueue.push({ index: item.index, questionId: '', value: item.value });
      }
      persistSession(buildPersistedState());
      step = 'exam';
      await tick();
    ...
    } catch (e: any) { startError = e.message ?? 'Network error'; }
  }
  ...
  function setupBeacon() {
    window.addEventListener('beforeunload', () => {
      ...
      for (const item of offlineQueue) {
        if (!item.questionId) continue;
        navigator.sendBeacon(`/api/exam/${data.examId}/answer`,
...
{:else if step === 'exam'}
  <KioskShell onViolation={handleViolation}>
    ...
    {#if violationVisible}
      <ViolationWarning
        count={violationCount} max={data.exam.maxViolations}
        action={violationAction} flagType={violationType}
    ...
    <div class="kiosk-layout">
      ...
      <aside class="left-panel">
        ...
        <div class="timer-block" class:warning={timerWarning} class:critical={timerCritical}>
          <div class="timer-bar-track">
            <div class="timer-bar-fill" style="width:{timerPct}%"
              class:warning={timerWarning} class:critical={timerCritical}></div>
          </div>
          ...
          <div class="nav-grid">
            {#each { length: totalQuestions } as _, i}
              <button
                ...
                onclick={() => void goToQuestion(i)} title="Question {i+1}" tabindex="-1"
              >{i+1}</button>
            {/each}
          </div>
        </div>
        ...
        {#if violationCount > 0}
          <div class="violation-counter" class:danger={violationCount >= data.exam.maxViolations - 1}>
            <AlertTriangle size={13} /> {violationCount}/{data.exam.maxViolations} violations
          </div>
        {/if}
      ...
      <main class="right-panel">
        {#if questionLoading}
          <div class="q-loading"><Loader2 size={36} class="spin" /><span>Loading question…</span></div>
        {:else if questionError}
          <div class="q-error">
            <AlertTriangle size={28} />
            <p>{questionError}</p>
            <button class="btn-retry" onclick={() => void fetchQuestion(currentIndex)}>Retry <kbd>R</kbd></button>
          </div>
        {:else if question}
          <div class="question-card">
            ...
            {#if question.type === 'mcq'}
              <div class="options-list">
                {#each question.options as opt, i}
                  ...
                  <button class="option-btn" class:selected onclick={() => saveAnswer(currentIndex, opt.id)} tabindex="-1">
            ...
            {#if question.type === 'fill_in_the_blank'}
              <div class="fitb-wrap">
                ...
                <input type="text" class="fitb-input" placeholder="Type your answer here…"
            ...
            <div class="question-footer">
              <button class="btn-nav btn-prev"
                disabled={currentIndex === 0 || questionLoading} onclick={goPrev} tabindex="-1">
              ...
              {#if currentIndex < totalQuestions - 1}
                <button class="btn-nav btn-next" disabled={questionLoading} onclick={goNext} tabindex="-1">
                  Next <kbd>N</kbd> <ChevronRight size={18} />
                </button>
              {:else}
                <button class="btn-nav btn-submit-footer"
                  disabled={isSubmitting} onclick={() => showSubmitConfirm = true} tabindex="-1">
      ...
      {:else if result}
        ...
        <div class="stats-grid">
          <div class="stat-cell">
            ...
            <span class="stat-val">{result.correct ?? '—'} / {result.totalQuestions ?? totalQuestions}</span>
          </div>
          <div class="stat-cell">
            <span class="stat-label">Answered</span>
            <span class="stat-val">{result.answered ?? answeredCount} / {result.totalQuestions ?? totalQuestions}</span>
          </div>
          <div class="stat-cell">
            <span class="stat-label">Score</span>
            <span class="stat-val">{result.score != null ? Number(result.score).toFixed(1) : '—'} / {data.exam.totalMarks}</span>
          </div>
          <div class="stat-cell">
            <span class="stat-label">Time taken</span>
            <span class="stat-val">{result.timeTakenSecs != null ? fmtSecs(result.timeTakenSecs) : '—'}</span>
          </div>
...
<style>
  :global(body) { margin: 0; padding: 0; overflow: hidden; user-select: none; background: var(--color-bg); }
  ...
  .splash-error {
    display: flex; align-items: center; gap: .5rem; font-size: .85rem; color: #dc2626;
  ...
  /* ── SSE badge ────────────────────────────────────────────────────────── */
  .sse-badge {
    position: fixed; top: .75rem; right: 1rem; z-index: 50;
    display: flex; align-items: center; gap: .4rem;
    font-size: .68rem; font-weight: 700; padding: .3rem .7rem;
  ...
  /* ── Layout ───────────────────────────────────────────────────────────── */
  .kiosk-layout { display: grid; grid-template-columns: 280px 1fr; height: 100vh; overflow: hidden; position: relative; z-index: 2; }

  /* ── Left panel ───────────────────────────────────────────────────────── */
  .left-panel {
    display: flex; flex-direction: column; gap: .875rem; padding: 1rem;
    background: var(--color-surface); border-right: 1px solid var(--color-border);
    overflow-y: auto; overflow-x: hidden;
  }

  .resume-badge {
    display: flex; align-items: center; gap: .4rem; font-size: .65rem; font-weight: 800;
    text-transform: uppercase; letter-spacing: .05em; padding: .3rem .65rem;
  ...
  .student-card {
    display: flex; align-items: center; gap: .625rem; padding: .625rem;
    background: var(--color-bg); border: 1px solid var(--color-border); border-radius: .75rem;
  }
  .student-photo { width: 38px; height: 38px; border-radius: 50%; object-fit: cover; border: 2px solid var(--accent, #10b981); flex-shrink: 0; }
  .student-photo-placeholder {
    width: 38px; height: 38px; border-radius: 50%; background: var(--color-bg);
  ...
  .student-name { font-size: .75rem; font-weight: 800; color: var(--color-text); white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
  .student-matric { font-size: .68rem; color: var(--color-muted); font-weight: 600; }

  .exam-info-block {
    display: flex; flex-direction: column; gap: .25rem; padding: .5rem .625rem;
  ...
  .exam-code-pill { font-size: .65rem; font-weight: 800; text-transform: uppercase; letter-spacing: .06em; color: var(--accent, #10b981); }
  .exam-title-text { font-size: .78rem; font-weight: 700; color: var(--color-text); line-height: 1.3; }

  .timer-block { padding: .75rem; background: var(--color-bg); border: 1.5px solid var(--color-border); border-radius: .75rem; transition: border-color .3s, background .3s; }
  ...
  .timer-bar-track { height: 5px; background: var(--color-border); border-radius: 3px; overflow: hidden; margin-bottom: .625rem; }
  .timer-bar-fill { height: 100%; background: var(--accent, #10b981); border-radius: 3px; transition: width 1s linear, background .3s; }
  ...
  .timer-digits { font-size: 1.6rem; font-weight: 900; font-variant-numeric: tabular-nums; letter-spacing: -.02em; line-height: 1; }
  .timer-block.warning  .timer-digits { color: #d97706; }
  .timer-block.critical .timer-digits { color: #dc2626; }
  .timer-hint { font-size: .68rem; font-weight: 700; margin-top: .375rem; padding: .2rem .5rem; border-radius: 4px; display: inline-block; }
  ...
  .progress-fill  { height: 100%; background: var(--accent, #10b981); border-radius: 3px; transition: width .4s ease; }

  .nav-section { display: flex; flex-direction: column; gap: .5rem; }
  .nav-legend { display: flex; align-items: center; gap: .5rem; font-size: .62rem; color: var(--color-muted); font-weight: 600; flex-wrap: wrap; }
  ...
  .nav-btn {
    ...
    background: var(--color-bg); color: var(--color-muted); font-size: .68rem; font-weight: 700;
  ...
  .nav-btn.answered {
    ...
    border-color: var(--accent, #10b981); color: var(--accent, #10b981);
  }
  .nav-btn.current {
    background: var(--accent-dark, #16a34a); border-color: var(--accent-dark, #16a34a); color: #fff;
    transform: scale(1.08); box-shadow: 0 2px 8px color-mix(in srgb, var(--accent, #10b981) 40%, transparent);
  }
  .nav-btn:hover:not(.current) { border-color: var(--accent, #10b981); color: var(--accent, #10b981); }

  .offline-bar, .syncing-bar { font-size: .7rem; font-weight: 700; text-align: center; padding: .4rem .75rem; border-radius: .5rem; }
  ...
  .violation-counter {
    display: flex; align-items: center; gap: .35rem; font-size: .72rem; font-weight: 700;
    padding: .35rem .75rem; border-radius: .5rem;
    background: color-mix(in srgb, #f59e0b 8%, var(--color-surface)); color: #92400e;
    border: 1px solid color-mix(in srgb, #f59e0b 30%, transparent);
  }
  .violation-counter.danger {
    background: color-mix(in srgb, #ef4444 8%, var(--color-surface)); color: #991b1b;
    border-color: color-mix(in srgb, #ef4444 30%, transparent);
  }

  .btn-submit-side {
    display: flex; align-items: center; justify-content: center; gap: .5rem; width: 100%; padding: .75rem;
    background: color-mix(in srgb, var(--accent, #10b981) 85%, #000); color: white; border: none;
    border-radius: .75rem; font-size: .85rem; font-weight: 800; cursor: pointer; font-family: inherit;
    transition: filter .2s;
  }
  .btn-submit-side:hover { filter: brightness(.88); }
  .btn-submit-side kbd { font-size: .62rem; padding: .1rem .35rem; background: rgba(255,255,255,.2); border-radius: 3px; }

  .key-hints {
    display: flex; flex-direction: column; gap: .3rem; padding: .625rem;
    background: var(--color-bg); border: 1px solid var(--color-border); border-radius: .625rem; margin-top: auto;
  }
  .key-row { font-size: .65rem; color: var(--color-muted); display: flex; align-items: center; gap: .25rem; flex-wrap: wrap; }
  .key-row kbd { font-size: .6rem; padding: .1rem .35rem; background: var(--color-surface); border: 1px solid var(--color-border); border-radius: 3px; color: var(--color-text); font-family: monospace; font-weight: 700; }

  /* ── Right panel ──────────────────────────────────────────────────────── */
  .right-panel { display: flex; flex-direction: column; height: 100vh; overflow: hidden; background: var(--color-bg); }
  .q-loading, .q-error { flex: 1; display: flex; flex-direction: column; align-items: center; justify-content: center; gap: 1rem; color: var(--color-muted); }
  ...
  .btn-retry {
    ...
    border-radius: .625rem; font-weight: 700; font-size: .875rem; cursor: pointer; font-family: inherit;
  }
  .question-card { flex: 1; display: flex; flex-direction: column; padding: 2rem 2.5rem; overflow-y: auto; gap: 1.5rem; }
  .question-header { display: flex; align-items: center; justify-content: space-between; flex-shrink: 0; }
  ...
  .q-marks { font-size: .78rem; font-weight: 700; padding: .25rem .65rem; background: color-mix(in srgb, var(--accent, #10b981) 12%, transparent); color: var(--accent, #10b981); border-radius: 6px; }
  .q-answered-badge { display: flex; align-items: center; gap: .3rem; font-size: .72rem; font-weight: 700; color: var(--accent, #10b981); background: color-mix(in srgb, var(--accent, #10b981) 10%, transparent); border: 1px solid var(--accent, #10b981); padding: .2rem .6rem; border-radius: 20px; }
  .question-body-wrap { flex-shrink: 0; }
  .question-text { font-size: 1.25rem; line-height: 1.75; color: var(--color-text); font-weight: 500; margin: 0; max-width: 72ch; }
  .question-img { max-width: 100%; max-height: 280px; object-fit: contain; border-radius: .75rem; margin-top: 1rem; border: 1px solid var(--color-border); }
  .options-list { display: flex; flex-direction: column; gap: .75rem; flex-shrink: 0; }
  .option-btn {
    display: flex; align-items: center; gap: 1rem; padding: 1rem 1.25rem;
    background: var(--color-surface); border: 1.5px solid var(--color-border);
    border-radius: .875rem; cursor: pointer; text-align: left; transition: all .15s;
    font-family: inherit; color: var(--color-text); max-width: 72ch;
  }
  .option-btn:hover:not(.selected) { border-color: var(--accent, #10b981); background: color-mix(in srgb, var(--accent, #10b981) 5%, var(--color-surface)); transform: translateX(2px); }
  .option-btn.selected { border-color: var(--accent, #10b981); background: color-mix(in srgb, var(--accent, #10b981) 10%, var(--color-surface)); }
  .option-letter { width: 36px; height: 36px; border-radius: 50%; border: 1.5px solid var(--color-border); display: flex; align-items: center; justify-content: center; font-size: .88rem; font-weight: 900; flex-shrink: 0; color: var(--color-muted); transition: all .15s; }
  ...
  .fitb-input { padding: 1rem 1.25rem; font-size: 1.1rem; font-family: inherit; color: var(--color-text); background: var(--color-surface); border: 1.5px solid var(--color-border); border-radius: .75rem; outline: none; transition: border-color .15s; user-select: text; }
  .fitb-input:focus { border-color: var(--accent, #10b981); }
  .question-footer { display: flex; align-items: center; justify-content: space-between; padding-top: 1rem; border-top: 1px solid var(--color-border); flex-shrink: 0; margin-top: auto; gap: 1rem; }
  .footer-center { flex: 1; text-align: center; }
  .offline-text { font-size: .78rem; color: #dc2626; font-weight: 600; }
  .btn-nav { display: inline-flex; align-items: center; gap: .5rem; padding: .75rem 1.5rem; border-radius: .75rem; font-size: .9rem; font-weight: 700; cursor: pointer; font-family: inherit; transition: all .2s; border: 1.5px solid var(--color-border); background: var(--color-surface); color: var(--color-text); }
  .btn-nav:disabled { opacity: .35; cursor: not-allowed; }
  .btn-nav kbd { font-size: .65rem; padding: .1rem .35rem; background: rgba(0,0,0,.06); border-radius: 3px; font-family: monospace; }
  .btn-prev:hover:not(:disabled) { border-color: var(--accent, #10b981); color: var(--accent, #10b981); }
  .btn-next { background: var(--accent, #10b981); color: white; border-color: var(--accent, #10b981); }
  .btn-next:hover:not(:disabled) { filter: brightness(.9); }
  .btn-next kbd { background: rgba(255,255,255,.2); }
  .btn-submit-footer { background: color-mix(in srgb, var(--accent, #10b981) 85%, #000); color: white; border-color: transparent; }
  .btn-submit-footer:hover:not(:disabled) { filter: brightness(.9); }
  .btn-submit-footer:disabled { opacity: .5; }
  .btn-submit-footer kbd { background: rgba(255,255,255,.2); }

  /* ── Modal ────────────────────────────────────────────────────────────── */
  .modal-backdrop { position: fixed; inset: 0; background: rgb(0 0 0/.65); backdrop-filter: blur(6px); display: flex; align-items: center; justify-content: center; z-index: 200; padding: 1rem; }
  .confirm-modal { background: var(--color-surface); border: 1px solid var(--color-border); border-radius: 1.25rem; padding: 2rem; width: 100%; max-width: 440px; display: flex; flex-direction: column; gap: 1rem; box-shadow: 0 24px 64px rgb(0 0 0/.2); }
  .confirm-modal h2 { font-size: 1.15rem; font-weight: 900; margin: 0; color: var(--color-text); }
  .confirm-warn { display: flex; align-items: center; gap: .5rem; font-size: .875rem; background: color-mix(in srgb, #f59e0b 8%, var(--color-surface)); color: #92400e; border: 1px solid color-mix(in srgb, #f59e0b 30%, transparent); padding: .625rem .875rem; border-radius: .5rem; margin: 0; }
  ...
  .confirm-keys kbd { padding: .1rem .4rem; background: var(--color-bg); border: 1px solid var(--color-border); border-radius: 3px; font-family: monospace; font-size: .7rem; }
  .confirm-actions { display: flex; gap: .75rem; justify-content: flex-end; }
  .btn-cancel { display: inline-flex; align-items: center; gap: .35rem; padding: .7rem 1.25rem; background: var(--color-bg); color: var(--color-text); border: 1.5px solid var(--color-border); border-radius: .625rem; font-weight: 700; font-size: .875rem; cursor: pointer; font-family: inherit; }
  .btn-cancel kbd { font-size: .65rem; padding: .1rem .3rem; background: var(--color-surface); border: 1px solid var(--color-border); border-radius: 3px; font-family: monospace; }
  .btn-cancel:disabled { opacity: .45; }
  .btn-submit-confirm { display: inline-flex; align-items: center; gap: .4rem; padding: .7rem 1.5rem; background: color-mix(in srgb, var(--accent, #10b981) 85%, #000); color: white; border: none; border-radius: .625rem; font-weight: 800; font-size: .875rem; cursor: pointer; font-family: inherit; transition: filter .2s; }
  .btn-submit-confirm:hover:not(:disabled) { filter: brightness(.88); }
  ...
  .terminal-close { font-size: .78rem; font-weight: 700; color: var(--color-muted); padding: .5rem 1rem; background: var(--color-surface); border: 1px solid var(--color-border); border-radius: .5rem; margin-top: .5rem; }
  ...
  @keyframes paused-ring { 0%,100% { opacity: .6; transform: scale(1); } 50% { opacity: 0; transform: scale(1.4); } }
  .terminal-card :global(.paused-icon) { color: #6366f1; }
  .paused-wait { display: flex; align-items: center; gap: .5rem; font-size: .82rem; color: var(--color-muted); background: var(--color-surface); border: 1px solid var(--color-border); padding: .625rem 1rem; border-radius: 8px; }

  /* Redirect countdown */
  .redirect-countdown {
    width: 100%; max-width: 360px; display: flex; flex-direction: column; gap: .5rem;
    align-items: center; font-size: .8rem; color: var(--color-muted);
  }
  .redirect-bar {
    width: 100%; height: 3px; background: var(--color-border); border-radius: 2px; overflow: hidden; position: relative;
  }
  .redirect-bar::after {
    content: ''; position: absolute; inset-block: 0; left: 0;
    background: var(--accent, #10b981); border-radius: 2px;
  ...
  .btn-go-home {
    display: inline-flex; align-items: center; gap: .4rem; padding: .65rem 1.25rem;
    background: var(--color-surface); color: var(--color-text); border: 1.5px solid var(--color-border);
    border-radius: .625rem; font-weight: 700; font-size: .875rem; cursor: pointer; font-family: inherit;
  ...
  .results-exam-info { display: flex; flex-direction: column; gap: .25rem; }
  .results-course { font-size: .68rem; font-weight: 800; text-transform: uppercase; letter-spacing: .06em; color: var(--accent, #10b981); }
  .results-title  { font-size: 1.1rem; font-weight: 900; color: var(--color-text); margin: 0; }
  .results-header :global(.done-icon) { color: var(--accent, #10b981); }

  .results-loading { display: flex; align-items: center; gap: .75rem; padding: 2.5rem; font-size: .9rem; color: var(--color-muted); justify-content: center; }
  .results-error   { display: flex; align-items: center; gap: .625rem; padding: 1.5rem; font-size: .88rem; color: #dc2626; background: color-mix(in srgb, #ef4444 6%, var(--color-surface)); }

  /* Score ring */
  .score-hero {
    display: flex; flex-direction: column; align-items: center; gap: .875rem;
    padding: 2rem 1.5rem; border-bottom: 1px solid var(--color-border);
  }
  .score-ring { position: relative; width: 140px; height: 140px; }
  .score-svg  { width: 100%; height: 100%; transform: rotate(-90deg); }
  .ring-bg   { fill: none; stroke: var(--color-border); stroke-width: 8; }
  .ring-fill { fill: none; stroke-width: 8; stroke-linecap: round; transition: stroke-dasharray .8s ease; }
  .score-inner {
    position: absolute; inset: 0;
    display: flex; flex-direction: column; align-items: center; justify-content: center; gap: .1rem;
  }
  .score-pct   { font-size: 2rem; font-weight: 900; color: var(--color-text); line-height: 1; letter-spacing: -.03em; }
  .score-grade { font-size: 1rem; font-weight: 800; }
  .score-verdict { display: flex; align-items: center; gap: .5rem; font-size: .95rem; font-weight: 800; }

  /* Stats */
  .stats-grid {
    display: grid; grid-template-columns: 1fr 1fr;
    gap: 0; border-bottom: 1px solid var(--color-border);
  }
  .stat-cell {
    display: flex; flex-direction: column; gap: .25rem;
    padding: 1rem 1.25rem; border-right: 1px solid var(--color-border);
    border-bottom: 1px solid var(--color-border);
  }
  .stat-cell:nth-child(even) { border-right: none; }
  .stat-cell:nth-last-child(-n+2) { border-bottom: none; }
  .stat-cell-warn { background: color-mix(in srgb, #f59e0b 5%, var(--color-surface)); }
  .stat-label { font-size: .7rem; font-weight: 700; text-transform: uppercase; letter-spacing: .05em; color: var(--color-muted); }
  .stat-val   { font-size: 1.1rem; font-weight: 900; color: var(--color-text); }
  .stat-val.warn { color: #d97706; }

  .results-footer { display: flex; gap: .75rem; padding: 1.25rem 1.5rem; justify-content: flex-end; }
  .results-home-btn { display: inline-flex; align-items: center; gap: .4rem; padding: .65rem 1.1rem; background: var(--color-bg); color: var(--color-text); border: 1.5px solid var(--color-border); border-radius: .625rem; font-weight: 700; font-size: .875rem; cursor: pointer; font-family: inherit; transition: background .15s; }
  .results-home-btn:hover { background: var(--color-border); }
  .btn-view-results { display: inline-flex; align-items: center; gap: .4rem; padding: .65rem 1.25rem; background: var(--accent, #10b981); color: white; border: none; border-radius: .625rem; font-weight: 700; font-size: .875rem; cursor: pointer; font-family: inherit; transition: filter .15s; }
  .btn-view-results:hover { filter: brightness(.9); }
