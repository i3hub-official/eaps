...
export const load: PageServerLoad = async (event) => {
  ...
  if (!faceDescriptor) {
    ...
    redirect(302, `/enroll?returnTo=${encodeURIComponent(returnUrl)}`);
  }

  const assessmentId = event.url.searchParams.get('examId'); // TODO: rename query param to assessmentId once the frontend is updated
