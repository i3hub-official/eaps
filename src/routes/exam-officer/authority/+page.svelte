<!-- src/routes/exam-officer/authority/+page.svelte -->
<script lang="ts">
	...
	import {
		...
		RotateCcw, Save, EyeOff, Layers, Filter,
		ChevronLeft, ChevronRight, UserCog, Clock
	} from '@lucide/svelte';
	...
	function scopeDescription(s: string | undefined) {
		return {
			...
			college_coordinator: 'You (Exam Officer) create exams for this course'
	...
	function getEligiblePeople(offeringId: string, scope: string) {
		const offering = data.offerings.find(o => o.id === offeringId);
		if (!offering) return [];

		const deptId = offering.departments[0]?.department.id;

		if (scope === 'lecturer') {
			if (!deptId) return [];
			const deptLecturers = data.departmentLecturers[deptId] || [];
			return deptLecturers.map(l => ({ ...l, role: 'Lecturer' }));
		} else if (scope === 'department_coordinator') {
			if (!deptId) return [];
			const coordinators = data.departmentCoordinators
				.filter(c => c.departmentId === deptId)
				.map(c => ({ ...c, role: 'Dept Coordinator' }));
			const hods = data.hodUsers
				.filter(h => h.departmentId === deptId)
				.map(h => ({ ...h, role: 'HOD' }));
			return [...coordinators, ...hods];
		} else if (scope === 'college_coordinator') {
			// For college_coordinator, we just need the exam officer themselves
			// The assignedUserId will be the exam officer's ID
			return [];
		}
		return [];
	}

	function getPersonCount(offeringId: string, scope: string) {
		if (scope === 'college_coordinator') return 1; // The exam officer themselves
		return getEligiblePeople(offeringId, scope).length;
	}
...
<svelte:head><title>Exam Authority Management — Exam Officer</title></svelte:head>
...
<div class="page">
	...
	<header class="page-header">
		<div class="header-left">
			<div class="header-icon"><ShieldCheck size={26} color="#0ea5e9" /></div>
			<div>
				...
				<p class="subtitle">
					...
					<span style="color:#0ea5e9">{scopeCounts().college_coordinator} you</span>
	...
	<div class="info-banner">
		<UserCog size={18} />
		<div>
			<strong>Your Authority:</strong>
			As an Exam Officer, you can assign who creates exams for each course.
			You can assign <strong>lecturers</strong>, <strong>department coordinators</strong>,
			or create exams yourself (<strong>Exam Officer</strong>).
		</div>
	</div>
	...
	<div class="cards-list">
		{#each paginatedOfferings() as o (o.id)}
			...
			{@const eligiblePeople = getEligiblePeople(o.id, scope)}
							...
							<div class="form-row">
								<div class="field">
									...
									>
										<option value="lecturer" selected={scope === 'lecturer'}>
											Lecturer (must teach the course)
										</option>
								...
								<div class="field">
									...
									{#if scope === 'college_coordinator'}
										<!-- For college_coordinator, assign to the exam officer themselves -->
										<input
											type="text"
											value="You (Exam Officer)"
											disabled
											class="field-disabled"
										/>
										<input type="hidden" name="assignedUserId" value={data.examOfficerId} />
									{:else}
										<select
											name="assignedUserId"
								...
								<div class="field field-submit">
									...
									<button
										...
										disabled={
											...
											(scope === 'college_coordinator' && !data.examOfficerId)
										}
									>
										<Save size={13} /> Apply
									</button>
