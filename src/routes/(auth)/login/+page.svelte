...
<script lang="ts">
  ...
  const roleTokens: Record<string, { primary: string; primaryHover: string; primaryLight: string }> = {
    admin:        { primary: '#6366f1', primaryHover: '#4f46e5', primaryLight: 'rgba(99,102,241,0.1)'  },
    lecturer:     { primary: '#8b5cf6', primaryHover: '#7c3aed', primaryLight: 'rgba(139,92,246,0.1)' },
    hod:          { primary: '#7c3aed', primaryHover: '#6d28d9', primaryLight: 'rgba(124,58,237,0.1)' },
    dean:         { primary: '#0ea5e9', primaryHover: '#0284c7', primaryLight: 'rgba(14,165,233,0.1)' },
    exam_officer: { primary: '#f97316', primaryHover: '#ea580c', primaryLight: 'rgba(249,115,22,0.1)' },
  ...
  const portalLabel = $derived(roleLabels[data.roleHint] ?? roleLabels.default);
...
<div
  class="root"
  style="--login-primary: {t.primary}; --login-primary-hover: {t.primaryHover}; --login-primary-light: {t.primaryLight};"
>
  <!-- Mobile hero strip -->
    ...
    <div class="form-wrap">
      ...
      {#if form?.error}
        <div class="error-box" role="alert">
          <div class="error-icon"><AlertCircle size={16} strokeWidth={2} /></div>
          ...
          <div class="field-group">
            ...
            <div class="field-wrap" class:has-error={emailTouched && emailError}>
              ...
              <input
                ...
                autocomplete="email" placeholder="you@mouau.edu.ng"
                class="field-input"
                bind:value={email}
            ...
            {#if emailTouched && emailError}
              <p class="inline-error" role="alert">
                <AlertCircle size={13} strokeWidth={2} /><span>{emailError}</span>
              </p>
            {/if}
          </div>
        ...
        {:else}
          ...
          <div class="field-group">
            ...
            <div class="field-wrap" class:has-error={passwordTouched && passwordError}>
              ...
              >
                {#if showPass}<EyeOff size={16} strokeWidth={1.5} />
                {:else}<Eye size={16} strokeWidth={1.5} />{/if}
              </button>
            </div>
            {#if passwordTouched && passwordError}
              <p class="inline-error" role="alert">
                <AlertCircle size={13} strokeWidth={2} /><span>{passwordError}</span>
              </p>
            {/if}
          </div>
      ...
      <div class="security-notice">
        <div class="security-icon"><ShieldCheck size={14} strokeWidth={2} /></div>
...
<style>
  ...
  :global(.dark) {
    --login-bg:            #0d1117;
    --login-bg-card:       #14171c;
    --login-text:          #e8edf5;
    --login-text-muted:    #8b95a5;
    --login-border:        #262a33;
  ...
  .mobile-glow {
    position: absolute;
    inset: 0;
    background: radial-gradient(ellipse at 20% 50%, color-mix(in srgb, var(--login-primary) 25%, transparent) 0%, transparent 60%);
    pointer-events: none;
  }
  ...
  .mobile-grid {
    ...
    inset: 0;
    background-image:
      linear-gradient(color-mix(in srgb, var(--login-primary) 4%, transparent) 1px, transparent 1px),
      linear-gradient(90deg, color-mix(in srgb, var(--login-primary) 4%, transparent) 1px, transparent 1px);
  ...
  .grid-pattern {
    ...
    inset: 0;
    background-image:
      linear-gradient(color-mix(in srgb, var(--login-primary) 3%, transparent) 1px, transparent 1px),
      linear-gradient(90deg, color-mix(in srgb, var(--login-primary) 3%, transparent) 1px, transparent 1px);
  ...
  .brand-icon {
    ...
    border-radius: 12px;
    background: color-mix(in srgb, var(--login-primary) 15%, transparent);
    border: 1px solid color-mix(in srgb, var(--login-primary) 25%, transparent);
  ...
  .brand-name {
    ...
    color: var(--login-text); margin: 0; letter-spacing: -0.01em;
  }

  .brand-sub {
    font-size: 0.6875rem; color: var(--login-text-muted);
    margin: 0.125rem 0 0; letter-spacing: 0.04em; font-weight: 500;
  }
  ...
  .gradient-text {
    background: linear-gradient(135deg, color-mix(in srgb, var(--login-primary) 60%, #34d399) 0%, var(--login-primary) 100%);
  ...
  .form-badge {
    ...
    color: var(--login-primary);
    background: var(--login-primary-light);
    border: 1px solid color-mix(in srgb, var(--login-primary) 20%, transparent);
  ...
  .form-title {
    ...
    color: var(--login-text); letter-spacing: -0.03em;
    margin: 0 0 0.5rem;
  }

  .form-hint {
    font-size: 0.8125rem; color: var(--login-text-muted);
  ...
  .steps {
    ...
    width: 220px; /* fixed width keeps it neat and centred */
  }
  ...
  .step.active .step-label { color: var(--login-text); }
  .step.done  .step-label { color: var(--login-primary); }

  .step-ring { padding: 3px; border-radius: 50%; transition: all 0.3s; }
  .step.active .step-ring {
    background: linear-gradient(135deg, var(--login-primary), color-mix(in srgb, var(--login-primary) 60%, #34d399));
  }
  ...
  .step.active .step-dot { border-color: transparent; color: #fff; background: transparent; }
  .step.done  .step-dot  { background: var(--login-primary); border-color: var(--login-primary); color: #fff; }
  ...
  .connector-fill {
    height: 100%; width: 0%;
    background: linear-gradient(90deg, var(--login-primary), color-mix(in srgb, var(--login-primary) 60%, #34d399));
  ...
  .error-box {
    ...
    padding: 0.875rem 1rem; border-radius: var(--login-radius);
    background: var(--login-error-light); border: 1px solid #fecaca;
    color: var(--login-error); font-size: 0.8125rem;
  ...
  .error-icon { flex-shrink: 0; opacity: 0.8; }
  ...
  .field-group { margin-bottom: 1rem; width: 100%; }
  ...
  .required { color: var(--login-error); font-size: 0.875rem; line-height: 1; }
  ...
  .field-icon {
    ...
    pointer-events: none; transition: color 0.2s; opacity: 0.6;
  }

  .field-wrap:focus-within .field-icon { color: var(--login-primary); opacity: 1; }
  .field-wrap.has-error    .field-icon { color: var(--login-error);   opacity: 1; }
  ...
  :global(.dark) .field-input:hover { border-color: #475569; }

  .field-input:focus {
    border-color: var(--login-primary);
    box-shadow: 0 0 0 4px color-mix(in srgb, var(--login-primary) 10%, transparent);
    background: var(--login-bg);
  }
  ...
  .field-input::placeholder { color: var(--login-text-muted); opacity: 0.5; font-weight: 400; }
  ...
  .pass-toggle:hover { color: var(--login-text); opacity: 1; background: var(--login-border); }

  .inline-error {
    display: flex; align-items: center; gap: 0.375rem;
    font-size: 0.75rem; color: var(--login-error);
    margin-top: 0.375rem;
    animation: slideDown 0.2s ease; font-weight: 500;
  }
  ...
  .text-link {
    ...
    color: var(--login-primary); text-decoration: none;
  ...
  .text-link::after {
    ...
    background: var(--login-primary); transition: width 0.2s;
  }

  .text-link:hover { color: var(--login-primary-hover); }
  ...
  .email-pill:hover {
    border-color: var(--login-primary);
    box-shadow: 0 0 0 3px color-mix(in srgb, var(--login-primary) 8%, transparent);
  }

  .pill-back {
    width: 28px; height: 28px; border-radius: 50%;
    background: var(--login-border); border: none; cursor: pointer;
    color: var(--login-text-muted);
    display: flex; align-items: center; justify-content: center;
    transition: all 0.15s; flex-shrink: 0;
  }

  .pill-back:hover { background: var(--login-primary); color: #fff; transform: scale(1.05); }

  .pill-content { display: flex; flex-direction: column; gap: 0.125rem; }

  .pill-label {
    font-size: 0.625rem; color: var(--login-text-muted);
    text-transform: uppercase; letter-spacing: 0.05em; font-weight: 600;
  }

  .pill-addr {
    font-size: 0.8125rem; font-weight: 600; color: var(--login-text);
  ...
  .btn-primary {
    ...
    color: #fff;
    box-shadow: 0 1px 3px 0 color-mix(in srgb, var(--login-primary) 30%, transparent),
                0 1px 2px -1px color-mix(in srgb, var(--login-primary) 20%, transparent);
  }
  ...
  .btn-primary:hover:not(:disabled) {
    transform: translateY(-1px);
    box-shadow: 0 10px 25px -5px color-mix(in srgb, var(--login-primary) 40%, transparent),
                0 4px 10px -5px color-mix(in srgb, var(--login-primary) 20%, transparent);
  }
  ...
  /* Spinner */
  .spinner-ring { display: inline-block; position: relative; width: 16px; height: 16px; }
  .spinner-ring span {
  ...
  .security-notice {
    ...
    background: var(--login-bg-card); border: 1px solid var(--login-border);
    border-radius: var(--login-radius);
    font-size: 0.6875rem; color: var(--login-text-muted); font-weight: 500;
    width: 100%;
  }

  .security-icon { color: var(--login-primary); opacity: 0.7; }

  /* ── Divider ─────────────────────────────────────────── */
  .divider { display: flex; align-items: center; gap: 0.75rem; margin: 1.25rem 0; width: 100%; }
  .divider-line { flex: 1; height: 1px; background: var(--login-border); }
  .divider-text { font-size: 0.6875rem; color: var(--login-text-muted); font-weight: 600; text-transform: uppercase; letter-spacing: 0.05em; }

  /* ── Register hint ───────────────────────────────────── */
  .register-hint {
    text-align: center; font-size: 0.8125rem;
    color: var(--login-text-muted); font-weight: 500; margin: 0;
  }

  .register-hint a { color: var(--login-primary); font-weight: 700; }
  .register-hint a:hover { color: var(--login-primary-hover); }

  /* ── Reduced motion ──────────────────────────────────── */
  @media (prefers-reduced-motion: reduce) {
    .glow-1, .glow-2, .spinner-ring span, .error-box, .inline-error,
    .feature-card, .field-input, .btn, .email-pill, .step-dot, .connector-fill {
