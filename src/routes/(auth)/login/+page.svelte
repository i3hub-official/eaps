...
<script lang="ts">
  ...
  // Client-side validation state (mirrors server error shape)
  ...
  function handleSubmit(e: SubmitEvent) {
    ...
    // Prevent submission if client errors exist
    if (emailErr || passErr) {
      e.preventDefault();
      return;
    }
  ...
  // Use client error if field is touched, otherwise fall back to server error
...
<div class="root">
  <!-- Left Panel (Green) - Visible on mobile as header -->
      ...
      <form method="POST" onsubmit={handleSubmit} class="form" novalidate>
        ...
        <div class="field">
          ...
          {#if getError('email')}
            <p class="inline-error">
              <AlertCircle size={14} /> {getError('email')}
            </p>
          {/if}
        </div>
        ...
        <div class="field">
          ...
          {#if getError('password')}
            <p class="inline-error">
              <AlertCircle size={14} /> {getError('password')}
            </p>
          {/if}
        </div>
...
<style>
  ...
  * { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  .field-input-wrap.error .field-icon {
    color: #dc2626;
  }
