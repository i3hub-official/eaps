<!-- src/routes/onboarding/staff/+page.svelte -->
<script lang="ts">
	...
	onMount(() => {
		// Extract token from URL hash (client-side, never sent to server in URL)
		...
		// Clear hash from address bar for security (removes token from history/referrer)
	...
	async function verifyToken() {
		...
		try {
			const response = await fetch('/api/staff/onboarding/verify', {
				method: 'POST',
				headers: { 'Content-Type': 'application/json' },
				body: JSON.stringify({ token }), // Token sent in request body, not URL
			})
	...
	async function handleProceed() {
		...
		try {
			...
			sessionStorage.setItem('staff_onboarding_id', invitation.id)

			// Redirect to setup form
			await goto('/onboarding/staff/setup')
		} catch (err) {
...
<div class="flex min-h-screen flex-col items-center justify-center bg-gradient-to-b from-background to-muted/30 p-4">
	<div class="w-full max-w-md">
		...
		<div class="mb-8 text-center">
			...
			<p class="mt-2 text-sm text-muted-foreground">Complete your profile to start using MOUAU e-Test</p>
		</div>
		...
		{:else if invitation}
			...
			<Card>
				<CardHeader>
					...
					<CardDescription>Your invitation is valid. Proceed to complete your profile.</CardDescription>
				</CardHeader>
				<CardContent class="space-y-5">
					...
					<div class="space-y-3 rounded-lg bg-muted/50 p-4">
						...
						<div class="flex gap-4">
							<div class="flex-1">
								<p class="text-xs font-medium uppercase tracking-wide text-muted-foreground">College</p>
						...
						{#if invitation.levels.length > 0}
							<div>
								<p class="text-xs font-medium uppercase tracking-wide text-muted-foreground">Assigned Levels</p>
					...
					<div class="rounded-lg border border-blue-500/30 bg-blue-500/5 p-3 text-xs text-blue-700 dark:text-blue-400">
						...
						<p>Your invitation token has been verified securely. You'll now set your password and complete your profile.</p>
					</div>
