<!-- src/routes/dean/exam-authority/+page.svelte -->
<script lang="ts">
  ...
  let bulkScope = $state<'' | 'lecturer' | 'department_coordinator' | 'college_coordinator'>('');
  let bulkOfficer = $state('');
  ...
  // ─── Derived ───
  const allDepts = $derived(() => {
    const set = new Set<string>();
    data.offerings.forEach(o => o.departments.forEach(d => set.add(d.department.name)));
  ...
  const filteredOfferings = $derived(() => {
    return data.offerings.filter(o => {
      ...
      const deptNames = o.departments.map(d => d.department.name);
      const matchesDept = deptFilter === 'all' || deptNames.includes(deptFilter);
      ...
      return matchesSearch && matchesDept && matchesScope && matchesPending && matchesLevel;
    });
  });
  ...
  const allOnPageSelected = $derived(() => {
    const ids = pageIds();
    return ids.size > 0 && Array.from(ids).every(id => selectedIds.has(id));
  });
  const selectedCount = $derived(() => Array.from(selectedIds).filter(id => selectableIds().has(id)).length);
...
<svelte:head><title>Exam Authority — Dean Portal</title></svelte:head>
...
<div class="page">
    ...
    <div class="header-right">
      {#if selectedCount() > 0}
        <span class="selection-pill">
          ...
          <button class="clear-pill" onclick={clearSelection} title="Clear selection"><X size={10} /></button>
        </span>
      {/if}
      <button
        class="btn-bulk"
        class:active={showBulkPanel}
  ...
  {#if form?.success}
    <div class="alert alert-success">
      <Check size={14} />
      <span>Authority updated successfully.</span>
    </div>
  {/if}
  ...
  <div class="filters-bar">
    <div class="filter-group">
      ...
      <select class="filter-select" bind:value={deptFilter}>
  ...
  <!-- ═══ Bulk Panel ═══ -->
  {#if showBulkPanel && selectedCount() > 0}
    <div class="bulk-panel">
      <div class="bulk-title">
        <Layers size={14} /> Bulk apply to <strong>{selectedCount()}</strong> selected
        {#if selectedCount() < totalItems()}
          <button class="btn-link-inline" onclick={selectAllFiltered}>
            Select all {totalItems()} matching filters
          </button>
        {/if}
      </div>
      <form
        method="POST"
        action="?/setAuthorityBulk"
        use:enhance={() => {
          return async ({ update }) => {
            await update();
            selectedIds = new Set();
  ...
  <div class="table-header">
    <label class="checkbox-wrap">
      <input
        ...
        disabled={paginatedOfferings().length === 0}
      />
    ...
    {#each paginatedOfferings() as o (o.id)}
      ...
      >
        ...
        <div class="card-row">
          ...
          <div class="cell-course" onclick={() => toggleExpand(o.id)}>
          ...
          <div class="cell-dept">
            <div class="dept-chips">
              {#each o.departments.slice(0, 2) as d}
                <span class="dept-chip">{d.department.name}</span>
              {/each}
              {#if o.departments.length > 2}
                <span class="dept-chip more">+{o.departments.length - 2}</span>
              {/if}
            </div>
          </div>
          ...
          <div class="cell-action">
            <button class="btn-edit" onclick={() => toggleExpand(o.id)}>
  ...
  {#if filteredOfferings().length > 0}
    <div class="footer-bar">
      ...
      {#if selectedCount() > 0}
        ...
        <span><strong>{selectedCount()}</strong> selected for bulk action</span>
      {/if}
    </div>
  {/if}
</div>
...
<style>
  ...
  .subtitle {
    font-size: 0.8rem; color: var(--color-muted); margin: 0.25rem 0 0;
  }
  .header-right {
  ...
  .clear-pill {
    ...
    color: #16a34a; padding: 0.1rem; border-radius: 50%;
    display: flex; align-items: center; justify-content: center;
  }
  .clear-pill:hover { background: rgba(0,0,0,0.08); }
  ...
  .alert {
    ...
    font-size: 0.85rem; animation: fadeUp 0.3s ease;
  }
  .alert-error {
  ...
  .search-input {
    ...
    font-size: 0.8rem; width: 260px; font-family: inherit;
  ...
  .search-clear {
    ...
    background: none; border: none; cursor: pointer;
    color: var(--color-muted); padding: 0.15rem; border-radius: 0.2rem;
  }
  ...
  .btn-reset {
    ...
    font-size: 0.75rem; color: var(--color-muted); cursor: pointer;
    padding: 0.3rem 0.5rem; border-radius: 0.3rem;
  }
  .btn-reset:hover { color: var(--color-text); background: var(--color-bg); }
  ...
  .level-chip:not(.active) .chip-count {
    background: var(--color-bg); opacity: 1;
  }
  ...
  .btn-link-inline {
    ...
    text-decoration: underline; padding: 0; margin-left: 0.4rem;
    font-family: inherit;
  }
  .bulk-form {
  ...
  .table-header {
    display: grid;
    grid-template-columns: 36px 2fr 1.2fr 140px 80px;
    gap: 0.75rem;
    align-items: center;
    padding: 0.6rem 1rem;
  ...
  .check-visual {
    ...
    border: 2px solid var(--color-border); border-radius: 0.25rem;
    background: var(--color-bg);
    transition: all 0.15s;
    display: flex; align-items: center; justify-content: center;
  }
  .checkbox-wrap input:checked + .check-visual {
    background: #3b82f6; border-color: #3b82f6;
  }
  .checkbox-wrap input:checked + .check-visual::after {
    content: ''; width: 5px; height: 8px;
    border: solid white; border-width: 0 2px 2px 0;
    transform: rotate(45deg) translate(-1px, -1px);
  }
  .checkbox-wrap input:focus + .check-visual {
  ...
  .expand-btn {
    ...
    border-radius: 0.25rem; transition: transform 0.2s;
  ...
  /* Status cell */
  .cell-status { display: flex; justify-content: flex-start; }
  .status-badge {
    display: inline-flex; align-items: center; gap: 0.3rem;
  ...
  .inline-form { padding-top: 0.875rem; }
  .form-row {
    display: flex; gap: 0.6rem; flex-wrap: wrap; align-items: flex-end;
  }
  .field {
  ...
  .field select, .field input[type='text'] {
    ...
    font-size: 0.8rem; font-family: inherit; min-width: 180px;
    transition: border-color 0.15s;
  }
  ...
  .btn {
    ...
    border-radius: 0.4rem; font-size: 0.8rem; font-weight: 600;
  ...
  .btn-link {
    background: none; border: none;
    color: #3b82f6; font-size: 0.85rem; cursor: pointer;
  ...
  .page-ellipsis {
    padding: 0 0.3rem; color: var(--color-muted); font-size: 0.78rem;
  }
  ...
  @keyframes flashGreen {
    0%   { background: rgba(34,197,94,0.15); border-color: #22c55e; }
    100% { background: transparent; border-color: var(--color-border); }
  }
  ...
  @media (max-width: 768px) {
    ...
    .cell-dept { display: none; }
    .cell-course { grid-area: course; }
