...
export const actions: Actions = {
  default: async ({ request, locals }) => {
    ...
    if (!departmentId) return fail(400, { error: 'Department is required for a HOD.' });
    if (alsoLectures && courseIds.length === 0) {
      return fail(400, { error: 'Assign at least one course if this HOD also lectures.' });
    }
