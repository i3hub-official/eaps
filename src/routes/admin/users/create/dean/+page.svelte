<!-- src/routes/admin/users/create/dean/+page.svelte -->
<script lang="ts">
  import type { PageData, ActionData } from './$types';
  import { enhance } from '$app/forms';
  import { onMount } from 'svelte';
  import { GraduationCap, AlertCircle, Copy, Check, RefreshCw, Eye, EyeOff, ChevronDown, Search, ArrowLeft } from '@lucide/svelte';
...
<div class="page" class:mounted>
  <div class="page-head">
    ...
    <div class="title-row">
      <div class="role-pill" style="background:rgba(16,185,129,.1);color:#10b981"><GraduationCap size={16} /> Dean</div>
      <h1>Create Dean</h1>
    </div>
    <p>Oversees an entire faculty. Read access to all exam schedules and results within the faculty.</p>
  </div>
  ...
  <form method="POST" use:enhance={() => { loading = true; return async ({ update }) => { loading = false; await update(); }; }}>
    ...
    <div class="card">
      ...
      <div class="grid">
        ...
        <div class="f half info-box">
          <div class="info-note">A Dean is scoped to a single faculty. They cannot manage users or create exams — only view schedules and results.</div>
        </div>
      </div>
    </div>
    ...
    <div class="foot">
      ...
      <button type="submit" class="btn-primary" disabled={loading || !password || !selectedCollegeId} style="background:#10b981">
        {#if loading}<RefreshCw size={13} class="spin" />{:else}<GraduationCap size={13} />{/if}
        Create Dean
      </button>
    </div>
  </form>
