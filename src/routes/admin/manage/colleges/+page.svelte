...
<script lang="ts">
  ...
  import { Building2, Plus, Pencil, Trash2, X, Check, AlertCircle, Search, Loader2 } from '@lucide/svelte';
...
<div class="manage-page">
  <div class="page-header">
    <div class="page-title">
      <Building2 size={22} class="title-icon" />
      <div>
        <h1>Colleges</h1>
        <p class="subtitle">{colleges.length} college{colleges.length !== 1 ? 's' : ''}</p>
      </div>
    </div>
      ...
      <table class="data-table">
        <thead>
          <tr>
            ...
            <th>Name</th>
            <th>Abbreviation</th>
            <th class="numeric">Departments</th>
            <th>Created</th>
        ...
        <tbody>
            ...
            <tr>
              <td colspan="6" class="empty">
                <div class="empty-state">
                  <Building2 size={40} class="empty-icon" />
                  <p class="empty-title">No colleges found</p>
                  <p class="empty-desc">{searchQuery ? 'Try adjusting your search terms.' : 'Get started by adding your first college.'}</p>
                  {#if !searchQuery}
          ...
          {:else}
            {#each colleges as college}
              <tr>
                <td><span class="badge code-badge">{college.code || '—'}</span></td>
                <td class="name-cell">{college.name}</td>
                <td>{college.abbreviation || '—'}</td>
                <td class="numeric"><span class="count-badge">{college._count?.departments ?? 0}</span></td>
                <td class="date-cell">{new Date(college.createdAt).toLocaleDateString('en-US', { year: 'numeric', month: 'short', day: 'numeric' })}</td>
                <td class="actions">
                  <div class="action-group">
                    <button class="action-btn edit" onclick={() => openEdit(college)} title="Edit college">
                      <Pencil size={15} />
                    </button>
                    <button class="action-btn delete" onclick={() => openDelete(college)} title="Delete college">
                      <Trash2 size={15} />
                    </button>
                  </div>
                </td>
      ...
      <div class="modal" onclick={(e) => e.stopPropagation()}>
        <div class="modal-header">
          <div class="modal-title-wrap">
            ...
            <h2 id="create-title">Add College</h2>
          </div>
        ...
        <form method="POST" action="?/create" use:enhance={handleEnhance('create')}>
          <div class="form-body">
            <div class="form-group">
              <label for="create-name">Name <span class="req">*</span></label>
              <input id="create-name" type="text" name="name" placeholder="e.g., College of Engineering" required />
            </div>
            <div class="form-row">
              <div class="form-group">
                <label for="create-code">Code</label>
                <input id="create-code" type="text" name="code" placeholder="e.g., COE" maxlength="10" />
              </div>
              <div class="form-group">
                <label for="create-abbr">Abbreviation</label>
                <input id="create-abbr" type="text" name="abbreviation" placeholder="e.g., Eng" maxlength="10" />
              </div>
            </div>
          </div>
      ...
      <div class="modal" onclick={(e) => e.stopPropagation()}>
        <div class="modal-header">
          <div class="modal-title-wrap">
            ...
            <h2 id="edit-title">Edit College</h2>
          </div>
          <button class="modal-close" onclick={closeModals} aria-label="Close modal"><X size={18} /></button>
        </div>
        <form method="POST" action="?/edit" use:enhance={handleEnhance('edit')}>
          ...
          <div class="form-body">
            <div class="form-group">
              ...
              <input id="edit-name" type="text" name="name" value={selectedCollege.name} required />
            </div>
            <div class="form-row">
              <div class="form-group">
                <label for="edit-code">Code</label>
                <input id="edit-code" type="text" name="code" value={selectedCollege.code ?? ''} maxlength="10" />
              </div>
              <div class="form-group">
                <label for="edit-abbr">Abbreviation</label>
                <input id="edit-abbr" type="text" name="abbreviation" value={selectedCollege.abbreviation ?? ''} maxlength="10" />
              </div>
            </div>
          </div>
          <div class="modal-footer">
  ...
  {#if showDeleteModal && selectedCollege}
    <div class="modal-overlay" onclick={closeModals} role="dialog" aria-modal="true" aria-labelledby="delete-title">
      <div class="modal delete-modal" onclick={(e) => e.stopPropagation()}>
        ...
        <h2 id="delete-title">Delete College?</h2>
        <p class="delete-target">Are you sure you want to delete <strong>{selectedCollege.name}</strong>?</p>
        <div class="warning-box">
          <AlertCircle size={16} />
          <span>This will also delete all departments and courses under this college. This action cannot be undone.</span>
        </div>
