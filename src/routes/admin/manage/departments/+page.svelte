...
<script lang="ts">
  ...
  import { Layers, Plus, Pencil, Trash2, X, Check, AlertCircle, Search, Loader2 } from 'lucide-svelte';
...
<div class="manage-page">
  <div class="page-header">
    <div class="page-title">
      <Layers size={22} class="title-icon" />
      <div>
        <h1>Departments</h1>
        <p class="subtitle">{departments.length} department{departments.length !== 1 ? 's' : ''} across all colleges</p>
      </div>
    </div>
  ...
  <div class="page-toolbar">
    <div class="search-box">
      <Search size={16} />
      <input type="text" placeholder="Search by name, code, or college..." bind:value={searchQuery} />
      {#if searchQuery}
        ...
        <tbody>
            ...
            <tr>
              <td colspan="6" class="empty">
                <div class="empty-state">
                  ...
                  <p class="empty-desc">{searchQuery ? 'Try adjusting your search terms.' : 'Get started by adding your first department.'}</p>
                  {#if !searchQuery}
          ...
          {:else}
            {#each departments as dept}
              <tr>
                <td><span class="badge code-badge">{dept.code}</span></td>
                <td class="name-cell">{dept.name}</td>
                <td><span class="college-badge">{dept.college?.name || '—'}</span></td>
                <td class="numeric"><span class="count-badge">{dept._count?.courses ?? 0}</span></td>
                <td class="date-cell">{new Date(dept.createdAt).toLocaleDateString('en-US', { year: 'numeric', month: 'short', day: 'numeric' })}</td>
                <td class="actions">
                  <div class="action-group">
                    <button class="action-btn edit" onclick={() => openEdit(dept)} title="Edit department">
                      <Pencil size={15} />
                    </button>
                    <button class="action-btn delete" onclick={() => openDelete(dept)} title="Delete department">
                      <Trash2 size={15} />
                    </button>
                  </div>
                </td>
      ...
      <div class="modal" onclick={(e) => e.stopPropagation()}>
        <div class="modal-header">
          <div class="modal-title-wrap">
            ...
            <h2 id="create-title">Add Department</h2>
          </div>
        ...
        <form method="POST" action="?/create" use:enhance={handleEnhance('create')}>
          <div class="form-body">
            <div class="form-group">
              ...
              <select id="create-college" name="collegeId" required>
            ...
            <div class="form-row">
              <div class="form-group">
                ...
                <input id="create-name" type="text" name="name" placeholder="e.g., Computer Science" required />
              </div>
              <div class="form-group">
                <label for="create-code">Code <span class="req">*</span></label>
                <input id="create-code" type="text" name="code" placeholder="e.g., CSC" required maxlength="10" />
              </div>
            </div>
          </div>
      ...
      <div class="modal" onclick={(e) => e.stopPropagation()}>
        <div class="modal-header">
          <div class="modal-title-wrap">
            ...
            <h2 id="edit-title">Edit Department</h2>
          </div>
          <button class="modal-close" onclick={closeModals} aria-label="Close modal"><X size={18} /></button>
        </div>
        <form method="POST" action="?/edit" use:enhance={handleEnhance('edit')}>
          ...
          <div class="form-body">
            <div class="form-group">
              ...
              <select id="edit-college" name="collegeId" required>
                <option value="">Select a college...</option>
                {#each data.colleges as college}
            ...
            <div class="form-row">
              <div class="form-group">
                ...
                <input id="edit-name" type="text" name="name" value={selectedDepartment.name} required />
              </div>
              <div class="form-group">
                <label for="edit-code">Code <span class="req">*</span></label>
                <input id="edit-code" type="text" name="code" value={selectedDepartment.code} required maxlength="10" />
              </div>
            </div>
          </div>
          <div class="modal-footer">
  ...
  {#if showDeleteModal && selectedDepartment}
    <div class="modal-overlay" onclick={closeModals} role="dialog" aria-modal="true" aria-labelledby="delete-title">
      <div class="modal delete-modal" onclick={(e) => e.stopPropagation()}>
        ...
        <h2 id="delete-title">Delete Department?</h2>
        <p class="delete-target">Are you sure you want to delete <strong>{selectedDepartment.name}</strong> ({selectedDepartment.code})?</p>
        <div class="warning-box">
          <AlertCircle size={16} />
          <span>This will also delete all courses under this department. This action cannot be undone.</span>
        </div>
