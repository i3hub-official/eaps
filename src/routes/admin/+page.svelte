...
<script lang="ts">
  ...
  // Chart rendering using simple CSS bars (no external chart lib needed, keeps it lightweight)
            ...
            {#each groups as g}
              ...
              <div class="pie-legend-item">
                ...
                <div class="pie-legend-info">
                  ...
                  <span class="pie-legend-value">{g.value} ({pct}%)</span>
                </div>
              </div>
            {/each}
          </div>
...
<style>
  ...
  .security-icon.blue { background: rgba(59, 130, 246, 0.1); color: #3b82f6; }
  .security-info {
    flex: 1;
    display: flex; flex-direction: column;
  }
  ...
  @media (max-width: 768px) {
    ...
    .dash-header-right { flex-direction: column; align-items: flex-start; }
    .charts-row { grid-template-columns: 1fr; }
